  ...
  for (let i = 0; i < selectorResults.length; i++) {
    ...
    if (showInfoEle.length > 0) {
      ...
      resultSearch.push({
        startPort: startPort
          ? startPort
          : cName("title blod")[index.startPort].innerText,
        endPort: endPort
          ? endPort
          : cName("title blod")[index.endPort].innerText,
        shipCompany: "OOCL",
        transferPort: cName("title blod")[index.transferPort].innerText,
        startPortPier: "",
        endPortPier: "",
        routeName: manifestRequest.route || "",
        code:
          cName("serviceCode")[index.code].innerText +
          ", " +
          cName("serviceCode")[index.code + 1].innerText,
        overTime: parseDateTime(dayEle[index.day + 3].innerText),
        use: 0,
        firstSupply: "",
        remark: "",
        remarkOp:
          trimArray(remarkOp[index.remarkOp].innerText.split(" "), 1, 0).join(
            " "
          ) +
          ", " +
          trimArray(
            remarkOp[index.remarkOp + 1].innerText.split(" "),
            1,
            0
          ).join(" "),
        ...
        gp20GP: hidePrice
          ...
          : convertStringToNumber(
              cName("e-single-number")[index.price].innerText
            ),
        gp40GP: hidePrice
          ? 88888
          : convertStringToNumber(
              cName("e-single-number")[index.price + 1].innerText
            ),
        gp40HQ: hidePrice
          ? 88888
          : convertStringToNumber(
              cName("e-single-number")[index.price + 2].innerText
            ),
      });
      ...
      resultSearch.push({
        ...
        remark: "",
        remarkOp: trimArray(
          remarkOp[index.remarkOp].innerText.split(" "),
          1,
          0
        ).join(" "),
