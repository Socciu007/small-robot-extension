...
function crawlData() {
  ...
  for (let i = 0; i < selectorResults.length; i++) {
    ...
    resultSearch.push({
      ...
      shippingSpace: "",
      gp20GP: convertStringToNumber(
        cName("e-single-number")[index.gp].innerText
      ),
      gp40GP: convertStringToNumber(
        cName("e-single-number")[index.gp + 1].innerText
      ),
      gp40HQ: convertStringToNumber(
        cName("e-single-number")[index.gp + 2].innerText
      ),
    });
  ...
  data = {
    search: {
      ...
      toPort: document.querySelectorAll(".current-search .from-to .title")[1]
        .innerText,
      cutoffDateStart: parseDateTime(
        document.querySelectorAll(".current-search .departure .text")[0]
          .innerText
      ),
      cutoffDateEnd: parseDateTime(
        document.querySelectorAll(".current-search .departure .text")[2]
          .innerText
      ),
      quantity: parseInt(
        document.querySelectorAll(".current-search .containers-count .text")[0]
          .innerText
      ),
