...
FyApp.controller('popupController', ['$scope', '$http', function popupController($scope, $http) {
  ...
  $scope.alert_message = { 'show': false, 'type': 'success', 'message': '错误提示' };
  $scope.closeAlert = function () {
    $scope.alert_message = { 'show': false, 'type': 'success', 'message': '没有错误' };
  };
  ...
  $scope.search = function () {
    setTimeout(() => {
      // 传输数据给目标页
      chrome.tabs.sendMessage($scope.working_tab_id, { 'data': $scope.inputValue }, res => { });
    }, 1000);
